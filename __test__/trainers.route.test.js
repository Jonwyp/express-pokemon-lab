...
describe("trainers", () => {
  ...
  describe("/trainers", () => {
    ...
    it("POST should not create new trainer if password length less than 8", async () => {
      const wrongTrainer = {
        username: "ash3",
        password: "1234567"
      };
  ...
  describe("/trainers/:username", () => {
    it("GET should respond with trainer details when login as correct trainer", async () => {
      const expectedTrainer = { username: "ash1" };
    ...
    it("GET should respond with incorrect trainer messager when login as incorrect trainer", async () => {
      const wrongTrainer = { username: "ash1" };
    ...
    it("GET should deny access if no token is provided", async () => {
      const { body: error } = await request(app)
        .get(`/trainers/ash2`)
        .expect(401);
